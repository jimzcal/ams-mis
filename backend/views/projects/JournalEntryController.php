...
class JournalEntryController extends Controller
{
        ...
        if(Yii::$app->user->can('addJournalEntry'))
        {
            ...
            if ($model->load(Yii::$app->request->post())) 
            {
                ...
                foreach ($model->sub_account as $key => $sub_account_value) 
                {
                    ...
                    $model_sub_ledger->jev_no = $model->jev_no;
    ...
    public function actionLoanpayment()
    {
        ...
        if($model->load(Yii::$app->request->post())) 
        {
            return $this->redirect(['paymentform', 'date' => $model->date_transaction, 'account_code' => $model->account_code, 'selection' => $model->selection, 'agency' => $model->agency]);
        }
    ...
    public function actionPaymentform($date, $account_code, $selection, $agency)
    {

        $dataProvider = $selection == "Single" ? SubLedgerAccounts::find()->where(['status' => 'Active'])->andWhere(['account_code' => $account_code])->all() : SubLedgerAccounts::find()->where(['status' => 'Active'])->andWhere(['mother_account' => '11210'])->all();
        ...
        if($model->load(Yii::$app->request->post())) 
        {
            if($model->total_credit != $model->total_debit)
                ...
                // var_dump($model->sub_loan_account);
                // exit();
                ...
                foreach ($model->sub_loan_account as $key => $value)
                {
                    ...
                    if(($model_payment->actual_remittance != 0.00) || ($model_payment->current_surcharge != 0.00))
                    {
