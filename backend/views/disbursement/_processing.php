        ...
        <div class="col-md-6">
            <div style="margin-right: auto; margin-left: auto; display: block;">
                <table style="width: 100%;" class="disbursement-table">
                    ...
                    <tr>
                        <td colspan="3">
                            <?= $form->field($model, 'status')->dropdownList(['Received' => 'Received', 'Cancelled' => 'Cancelled', 'Processed' => 'Processed', 'Verified' => 'Verified', 'For Release' => 'For Release', 'Return to payee' => 'Return to payee']) ?>
                        </td>
                    </tr>
                </table>
                ...
                <div>
                    <?php foreach ($attachments as $val) : ?>
                        <?php if($val != '') : ?>
                            ...
                            <label style="font-size: 10px;"><?= mb_strimwidth($val, 0, 50, ' ...') ?></label><br>
                        <?php endif ?>
                    <?php endforeach ?>

                    <?php foreach ($lacking as $lack) : ?>
                        <input type="checkbox" name="requirements[<?= $lack ?>]" value="<?= $lack ?>">
                        <label style="font-size: 10px;"><?= mb_strimwidth($lack, 0, 50, ' ...') ?></label><br>
                    <?php endforeach ?>
                </div>
            </div>
        </div>
